# Retail-Sales-Inventory-Analysis-Dashboard
This project analyzes retail sales and inventory data to identify sales trends, inventory gaps, and stock optimization opportunities.
---

## Overview
This project demonstrates how Excel can be used to build an interactive business intelligence dashboard for retail inventory management.
The dashboard analyzes more than 73,000 retail transactions and provides insights into revenue, inventory performance, product demand, and operational efficiency.

## Business Value

This dashboard helps retail managers:
* Monitor inventory health in real time.
* Identify overstock and stockout risks.
* Track revenue performance across categories and regions.
* Analyze top-performing products.
* Support data-driven inventory planning and replenishment decisions.

## Business Questions
- Which products generate the most revenue?
- Which products need restocking?
- Which products have excess inventory?
- How does demand compare with inventory levels?
  
## Tools Used
* **Microsoft Excel**
* **SQL**
* **Power Query**
* **PivotTables**
* **PivotCharts**
* **Dashboard Features**

## KPIs
- Total Revenue
- Units Sold
- Inventory Coverage Ratio
- Inventory Status
- Sales Performance by Category

## Analysis
- Cleaned and validated retail data
- Created inventory classification logic
- Analyzed demand forecast and inventory levels
- Built interactive dashboard for decision-making

---

## The dashboard includes interactive slicers that allow users to filter data by:
*Region *Store *Product *Product Category *Weather Condition *Date *KPIs *Revenue *Units Sold *Inventory Level *Restock Alerts *Average Inventory Coverage Ratio *Visualizations *Revenue Over Time *Revenue by Category *Top Selling Products *Inventory Status Distribution *Revenue by Region *Weather Impact on Units Sold *Weather Impact on Inventory Level

---

## Key Business Insights

* Total Revenue reached $495.67M.
* Nearly 10 million units were sold during the analysis period.
* 49% of products maintained healthy inventory levels.
* 30% of inventory was classified as overstocked, while 18% represented excess inventory, indicating opportunities to optimize inventory investment.
* 2,585 products required restocking, helping identify potential stockout risks.
* Furniture generated the highest revenue among all product categories.
* Monthly revenue remained relatively stable throughout the year.
* Weather conditions showed only minor variation in units sold and inventory levels.

